...
void *
__iCrt_aligned_offset_realloc (void *ptr, size_t sz, size_t align, size_t off)
{
  void *tmp, **savep;
  size_t pad_old, pad_new, sz_old;

  if (!ptr)
    return __iCrt_aligned_offset_malloc (sz, align, off);
  ...
  tmp = __iCrt_realloc (*savep, sz + align + sizeof (void *));
