...
static void __cdecl
l_qsort (char *l, char *r, size_t sz, char *hswap, int (__cdecl *fcmp) (const void *, const void *))
{
  char *p, *max;
...
void __cdecl
ic_qsort (void *addr, size_t n, size_t sz, int (__cdecl *fcmp) (const void *, const void *))
{
	  ...
	  if (c <= l_p)
	    {
	      do 
		l_p += sz;
	      while (l_p <= r && (*fcmp) (l_p, c) <= 0);
	    }

	  do
	    r_p -= sz;
	  while (r_p > c && (*fcmp) (r_p, c) > 0);
