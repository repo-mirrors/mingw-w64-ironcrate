...
/* int __cdecl _strnicmp (const char *s1, const char *s2, size_t n).
   ...
   (2): Otherwise call _strnicmp_l with NULL as locale.
...
ic_test_main (int seq, int flags)
{
  ...
  switch (seq)
  {
    case 0:
      ...
      IC_TEST_CHECK ((*fct)("def", NULL, 2) == 0x7fffffff && errno == EINVAL);
      IC_TEST_CHECK ((*fct)(NULL, "abc", 2) == 0x7fffffff && errno == EINVAL);
      IC_TEST_CHECK ((*fct)(NULL, NULL, 2) == 0x7fffffff && errno == EINVAL);
      ic_test_reset_internal_vars ();
      ...
      IC_TEST_CHECK ((r = (*fct)("ABCD", "abcE", 3)) == 0);
