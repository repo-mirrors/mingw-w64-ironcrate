...
/* int __cdecl _strnicoll_l (const char *s1, const char *s2, size_t n,
			     _locale_t pli).
   ...
   (2): Otherwise if LC_CTYPE isn't set use comparison _strnicmp.
   (3): Else compare strings case-insensitive via multibyte comparison of OS.
...
ic_test_main (int seq, int flags)
{
  ...
  int (*fct)(const void *, const void *, size_t, _locale_t) =
	ic_test_getsymbol ("_strnicoll_l");
  if (!fct)
