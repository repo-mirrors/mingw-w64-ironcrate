...
/* size_t __cdecl strspn (const char *s, const char *c);


   Flow-graph:
   (1): Use a bit map of 256 bits to indicate characters described in c.
   (2): Return the number of characters in string s from left to right, which are indicated
   	in map.
...
ic_test_main (int seq, int flags)
{
  ...
  switch (seq)
  {
    case 0:
      ...
      IC_TEST_CHECK ((*fct)("", "") == 0);
      IC_TEST_CHECK ((*fct)("123", "abc") == 0);
      IC_TEST_CHECK ((*fct)("12a3", "abc") == 0);
      IC_TEST_CHECK ((*fct)("b12a3", "abc") == 1);
      IC_TEST_CHECK ((*fct)("123123", "123") == 6);
