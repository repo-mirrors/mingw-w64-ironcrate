...
/* int __cdecl _strnicoll (const char *s1, const char *s2, size_t n).
   ...
   (1): If s1 or s2 are NULL and the locale wasn't changed, then set errno to EINVAL,
   	...
   	do comparision via _strnicmp.
   (2): Otherwise call _strnicoll_l with NULL as locale.
